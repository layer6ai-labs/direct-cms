...
from scheduler_euler import EulerScheduler
from scheduler_heun import HeunScheduler
...
def log_testing(vae, args, writer, device, weight_dtype):
    ...
    if args.scheduler == "ddim":
        ...
        Scheduler = HeunScheduler

    pipeline = StableDiffusionXLPipeline.from_pretrained(
        args.pretrained_teacher_model,
        vae=vae,
        scheduler=Scheduler.from_pretrained(args.pretrained_teacher_model, subfolder="scheduler"),
    ...
    for _, prompt in tqdm(enumerate(validation_prompts)):
        ...
        with autocast_ctx:
            images = pipeline(
                prompt=prompt,
                num_inference_steps=args.num_original_inference_steps,
    ...
    train_gen_images_dir = Path(args.output_dir, "train_gen_images", f"{args.num_original_inference_steps}_steps", f"{args.guidance_scale}_gscale")
    ...
    tag_suffix = f"[{args.num_original_inference_steps} steps] [{args.guidance_scale} guidance scale]"
