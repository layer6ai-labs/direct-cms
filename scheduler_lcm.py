...
class LCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    Args:
        ...
        scale_boundary (`bool`, defaults to `True`):
            Whether the boundary scalings are to be used or not. This is set to True by default.
    ...
    def scalings_for_boundary_conditions(self, timestep):
        ...
        scaled_timestep = self.config.timestep_scaling * timestep
    ...
    def step(
        ...
        if self.config.scale_boundary:
            c_skip, c_out = self.scalings_for_boundary_conditions(timestep)
            pred_x_0 = c_skip * sample + c_out * pred_x_0
        ...
        if step_index != self.num_inference_steps - 1:
            ...
            noise = randn_tensor(
                model_output.shape, generator=generator, device=model_output.device, dtype=pred_x_0.dtype
            )
            prev_x = alpha_prev * pred_x_0 + sigma_prev * noise
        else:
            prev_x = pred_x_0
        ...
        return LCMSchedulerOutput(prev_sample=prev_x, pred_original_sample=pred_x_0)
