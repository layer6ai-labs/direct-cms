...
class DDIMSolver:
    ...
    def step(self, x_t, pred_noise, timestep_index, start_timesteps):
        start_timestep = self.start_timesteps[timestep_index]
        alpha_start = self.ddim_alphas_start[timestep_index]
        alpha_next = self.ddim_alphas_next[timestep_index]
        sigma_start = self.ddim_sigmas_start[timestep_index]
        sigma_next = self.ddim_sigmas_next[timestep_index]
        pred_x_0 = (x_t - sigma_start * pred_noise) / alpha_start
        dir_x_t = sigma_next * pred_noise
        x_next = alpha_next * pred_x_0 + dir_x_t
        # Do not update x_t's who's initial condition is earlier in time than the current timestep
        no_step = start_timestep > start_timesteps
        x_next[no_step] = x_t[no_step].to(x_next.dtype)
        return None, x_next
...
class EulerSolver:
    ...
    def step(self, x_t, pred_noise, timestep_index, start_timesteps):
        start_timestep = self.start_timesteps[timestep_index]
        next_timestep = self.next_timesteps[timestep_index]
        d_t = float(next_timestep - start_timestep) / self.num_train_timesteps
        beta = self.euler_betas[timestep_index]
        sigma = self.euler_sigmas[timestep_index]
        x_t_coeff = -0.5 * beta
...
class HeunSolver:
    ...
    def step(self, x_t, derivative_t, pred_x_next, pred_noise_next, timestep_index, start_timesteps):
        start_timestep = self.start_timesteps[timestep_index]
        next_timestep = self.next_timesteps[timestep_index]
        d_t = float(next_timestep - start_timestep) / self.num_train_timesteps
        beta = self.heun_betas[timestep_index]
        sigma = self.heun_sigmas[timestep_index]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                start_timesteps = torch.from_numpy(solver.start_timesteps).to(latents.device).long()[index]
                ...
                latents = noise_scheduler.add_noise(latents, noise, start_timesteps - 1)
                ...
                # 6. Solve the augmented PF-ODE using the teacher model. For each solver step, compute
                # conditional and unconditional teacher model samples to get CFG estimates of the predicted
                # noise, then run the ODE solver using these estimates to predict the data points in the
                ...
                with torch.no_grad():
                    unet.eval()
                    # 7. Find the in-batch maximum timestep to start solving the ODE from
                    start_timestep_index = int(index.min().cpu().item())  # smallest index corresponds to largest timestep
                    num_discr_timesteps_batch = solver.num_discr_timesteps - start_timestep_index  # only need this many ODE steps
                    noisy_model_input = latents.clone()
                    for i in range(num_discr_timesteps_batch):
                        start_timestep = solver.start_timesteps[start_timestep_index + i]
                        start_timestep = torch.LongTensor([start_timestep]*bsz).to(noisy_model_input.device)

                        # 8. Get teacher model prediction on noisy_model_input and conditional embedding c
                        ...
                        # 9. Get teacher model prediction on noisy_model_input and unconditional embedding 0
                        ...
                        # 10. Calculate the CFG estimate of pred_noise
                        # Note that this uses the Imagen CFG formulation
                        pred_noise = uncond_pred_noise + w * (cond_pred_noise - uncond_pred_noise)

                        # 11. Run one step of the ODE solver to estimate the next point on the augmented
                        # PF-ODE trajectory
                        derivative, noisy_model_input_next = solver.step(noisy_model_input, pred_noise, start_timestep_index + i, start_timesteps)

                        if args.solver == "ddim" or args.solver == "euler":
                            noisy_model_input = noisy_model_input_next.to(accelerator.unwrap_model(unet).dtype)
                        elif args.solver == "heun":
                            # In a Heun step, we treat a Euler step as a predictor step and then run a corrector step
                            next_timestep = heun_solver.next_timesteps[start_timestep_index + i]
                            next_timestep = torch.LongTensor([next_timestep]*bsz).to(noisy_model_input.device)

                            # 12. Get teacher model prediction on Euler predicted noisy_model_input and conditional embedding c
                            ...
                            # 13. Get teacher model prediction on Euler predicted noisy_model_input and unconditional embedding 0
                            ...
                            # 14. Calculate the CFG estimate of pred_noise_next
                            # Note that this uses the Imagen CFG formulation
                            pred_noise_next = uncond_pred_noise_next + w * (cond_pred_noise_next - uncond_pred_noise_next)

                            # 15. Run one step of the Heun ODE solver to estimate the next point on the augmented
                            # PF-ODE trajectory by correcting the previous Euler step
                            noisy_model_input = heun_solver.step(noisy_model_input, derivative, noisy_model_input_next, pred_noise_next, start_timestep_index + i, start_timesteps).to(accelerator.unwrap_model(unet).dtype)

                        target_pred_x_0 = noisy_model_input
                ...
                # 16. Get online prediction on noisy_model_input
                noise_pred = unet(
                    latents,
                    start_timesteps,
                    encoder_hidden_states=prompt_embeds,
                ...
                alphas = extract_into_tensor(alpha_schedule, start_timesteps - 1, latents.shape)
                sigmas = extract_into_tensor(sigma_schedule, start_timesteps - 1, latents.shape)
                pred_x_0 = (latents - sigmas * noise_pred) / alphas

                # 17. Get boundary scalings for start timesteps and apply to predicted x_0
                c_skip, c_out = scalings_for_boundary_conditions(
                    start_timesteps, timestep_scaling=args.timestep_scaling_factor
                )
                c_skip, c_out = [append_dims(x, latents.ndim) for x in [c_skip, c_out]]

                pred_x_0 = c_skip * latents + c_out * pred_x_0
                ...
                # 19. Backpropagate on the online student model (`unet`) (only LoRA)
